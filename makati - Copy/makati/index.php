...
$timeout_duration = 30; // 10 sec (in seconds)
$inactivity_limit = 30; // 10 sec (in seconds)
...
$loginQuery = "SELECT * FROM makati_activity_logs WHERE action = 'Login' ORDER BY timestamp DESC LIMIT 5";
$loginResult = mysqli_query($con, $loginQuery);
$loginHistory = mysqli_fetch_all($loginResult, MYSQLI_ASSOC);

$productQuery = "SELECT * FROM makati_activity_logs WHERE action = 'Add Item' ORDER BY timestamp DESC LIMIT 5";
$productResult = mysqli_query($con, $productQuery);
$productHistory = mysqli_fetch_all($productResult, MYSQLI_ASSOC);

$removeQuery = "SELECT * FROM makati_activity_logs WHERE action = 'Remove Item' ORDER BY timestamp DESC LIMIT 5";
$removeResult = mysqli_query($con, $removeQuery);
$removeHistory = mysqli_fetch_all($removeResult, MYSQLI_ASSOC);

$updateQuery = "SELECT * FROM makati_activity_logs WHERE action = 'Update Item' ORDER BY timestamp DESC LIMIT 5";
$updateResult = mysqli_query($con, $updateQuery);
$updateHistory = mysqli_fetch_all($updateResult, MYSQLI_ASSOC);

$viewQuery = "SELECT * FROM makati_activity_logs WHERE action = 'View Item' ORDER BY timestamp DESC LIMIT 5";
$viewResult = mysqli_query($con, $viewQuery);
$viewHistory = mysqli_fetch_all($viewResult, MYSQLI_ASSOC);

$viewCustomerQuery = "SELECT * FROM makati_activity_logs WHERE action = 'View Customer' ORDER BY timestamp DESC LIMIT 5";
...
$releaseCustomerQuery = "SELECT * FROM makati_activity_logs WHERE action = 'Release Item' ORDER BY timestamp DESC LIMIT 5";
...
$unreadCountResult = mysqli_query($con, "SELECT COUNT(*) AS total FROM makati_activity_logs WHERE is_viewed = FALSE");
...
<head>
  ...
  <title>Makati Branch Admin</title>

  <link rel="stylesheet" href=" css/makati_main.css">
  ...
  <link rel="stylesheet" href="makati/css/modal.css">
</head>
...
<body>
  ...
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <ul class="nav-list primary-nav">
        ...
        <li class="nav-item">
          <a href="?page=makati_dashboard" class="nav-link">
            <span class="nav-icon material-symbols-rounded">dashboard</span>
            <span class="nav-label">Makati Dashboard</span>
          </a>
          <span class="nav-tooltip">Makati Dashboard</span>
        </li>

        <li class="nav-item">
          <a href="?page=makati_products" class="nav-link ">
            <i class="fa-solid fa-location-dot dotted"></i>
            <span class="nav-label">Makati Products</span>
          </a>
          <span class="nav-tooltip">Makati Products</span>
        </li>

        <li class="nav-item">
          <a href="?page=makati_add_products_section" class="nav-link ">
        ...
        <li class="nav-item">
          <a href="?page=makati_release_products" class="nav-link ">
        ...
        <li class="nav-item">
          <a href="?page=makati_ctm_records" class="nav-link ">
      ...
      <ul class="nav-list secondary-nav">
        ...
        <li class="nav-item">
          <a href="?page=makati_history" class="nav-link ">
            <i class="fa-solid fa-clock-rotate-left dotted"></i>
            <span class="nav-label">Makati History </span>
          </a>
          <span class="nav-tooltip">Makati History </span>
        </li>

        <li class="nav-item">
          <a href="../main/chat/chat.php" class="nav-link">
  ...
  <div class="main">
    ...
    switch ($page) {
      ...
      case 'makati_dashboard':
        include 'makati_dashboard.php';
        break;

      case 'makati_products':
        include 'mkt_prod_tbl.php';
        break;

      case 'makati_add_products_section':
        include 'mkt_add_prod.php';
        break;

      case 'makati_release_products':
        include 'mkt_release.php';
        break;

      case 'makati_ctm_records':
        include 'mkt_records.php';
        break;

      case 'makati_history':
        include 'mkt_history.php';
        break;
  ...
  <script>
    ...
    window.addEventListener("beforeunload", function () {
      ...
      if (!isNavigatingAway && !isReload && !isBackForward) {
        navigator.sendBeacon("makati/logout-on-close.php");
      }
    });
  </script>

  <script src="makati/js/jquery.js"></script>
  <script src="makati/js/main.js"></script>
