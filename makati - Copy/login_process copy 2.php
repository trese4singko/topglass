...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    function handleLogin($con, $row, $table)
    {
        ...
        logmktActivity($con, $row['username'], 'Login', null, "Admin user {$row['username']} has logged in Makati Branch");
        logActivity($con, $row['username'], 'Login', null, "Admin user {$row['username']} has logged in Makati Branch");

        echo json_encode([
            "status" => "success",
            "redirect" => "makati/index.php?page=makati_dashboard",
            "session_token" => $token
        ]);
        exit();
    }
    ...
    function checkLoginTable($con, $table)
    {
        ...
        if ($res && mysqli_num_rows($res) > 0) {
            ...
            if ($row['usertype'] !== 'Makati Admin') {
                echo json_encode(["status" => "unauthorized", "message" => "Access restricted. For Makati admins/Employees only."]);
                exit();
            }
