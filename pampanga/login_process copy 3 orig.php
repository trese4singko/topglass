...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    function handleLogin($con, $row, $table)
    {
        ...
        logppgActivity($con, $row['username'], 'Login', null, "Admin user {$row['username']} has logged-in in Pampanga Branch");
        logActivity($con, $row['username'], 'Login', null, "Admin user {$row['username']} has logged-in in Pampanga Branch");
    ...
    function checkLoginTable($con, $table)
    {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if ($row['usertype'] !== 'Pampanga Admin') {
                echo json_encode(["status" => "unauthorized", "message" => "Access restricted. Allowed for Pampanga Admins/Employees only."]);
                exit();
            }
