<?php
session_start();
include "pampanga/functions.php";
include "pampanga/ppg_log_functions.php";
...
$lockout_duration = 20; // 20 seconds lockout
$max_attempts = 3;
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_SESSION['lockout_until']) && time() < $_SESSION['lockout_until']) {
        ...
        echo json_encode(["status" => "locked", "message" => "Too many failed attempts. Try again in " . gmdate("i\m s\s", $remain)]);
        exit();
    }
    ...
    function handleLogin($con, $row, $table)
    {
        ...
        logppgActivity($con, $row['username'], 'Login', null, "Admin user {$row['username']} has logged in Pampanga Branch");
        logActivity($con, $row['username'], 'Login', null, "Admin user {$row['username']} has logged in Pampanga Branch");

        echo json_encode([
            "status" => "success",
            "redirect" => "pampanga/index.php?page=pampanga_dashboard",
            "session_token" => $token
        ]);
        exit();
    }
    ...
    function checkLoginTable($con, $table)
    {
        ...
        $res = mysqli_query($con, "SELECT * FROM $table WHERE username = '$username'");
        if ($res && mysqli_num_rows($res) > 0) {
            $row = mysqli_fetch_assoc($res);

            if ($row['usertype'] !== 'Pampanga Admin') {
                echo json_encode(["status" => "unauthorized", "message" => "Access restricted. For Pampanga admins/Employees only."]);
                exit();
            }
            ...
            if (password_verify($password, $row['password'])) {
                ...
                if ($_SESSION['failed_attempts'] >= $max_attempts) {
                    $_SESSION['lockout_until'] = time() + $lockout_duration; // 20 seconds lockout
                    echo json_encode(["status" => "locked", "message" => "Too many failed attempts. Wait 20 seconds."]);
                    exit();
                }
