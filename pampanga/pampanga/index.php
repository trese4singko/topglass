...
$loginQuery = "SELECT * FROM ppg_activity_logs WHERE action = 'Login' ORDER BY timestamp DESC LIMIT 5";
$loginResult = mysqli_query($con, $loginQuery);
$loginHistory = mysqli_fetch_all($loginResult, MYSQLI_ASSOC);

$productQuery = "SELECT * FROM ppg_activity_logs WHERE action = 'Add Item' ORDER BY timestamp DESC LIMIT 5";
$productResult = mysqli_query($con, $productQuery);
$productHistory = mysqli_fetch_all($productResult, MYSQLI_ASSOC);

$removeQuery = "SELECT * FROM ppg_activity_logs WHERE action = 'Remove Item' ORDER BY timestamp DESC LIMIT 5";
$removeResult = mysqli_query($con, $removeQuery);
$removeHistory = mysqli_fetch_all($removeResult, MYSQLI_ASSOC);

$updateQuery = "SELECT * FROM ppg_activity_logs WHERE action = 'Update Item' ORDER BY timestamp DESC LIMIT 5";
$updateResult = mysqli_query($con, $updateQuery);
$updateHistory = mysqli_fetch_all($updateResult, MYSQLI_ASSOC);

$viewQuery = "SELECT * FROM ppg_activity_logs WHERE action = 'View Item' ORDER BY timestamp DESC LIMIT 5";
$viewResult = mysqli_query($con, $viewQuery);
$viewHistory = mysqli_fetch_all($viewResult, MYSQLI_ASSOC);

// Fetching view customer logs
$viewCustomerQuery = "SELECT * FROM ppg_activity_logs WHERE action = 'View Customer' ORDER BY timestamp DESC LIMIT 5";
...
$releaseCustomerQuery = "SELECT * FROM ppg_activity_logs WHERE action = 'Release Item' ORDER BY timestamp DESC LIMIT 5";
$releaseCustomerResult = mysqli_query($con, $viewCustomerQuery);
$releaseCustomerHistory = mysqli_fetch_all($viewCustomerResult, MYSQLI_ASSOC);
...
<head>
  ...
  <link rel="stylesheet" href="css/pmpng_main.css">
...
<body>
  ...
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <ul class="nav-list primary-nav">
        ...
        <li class="nav-item">
          <a href="?page=pampanga_dashboard" class="nav-link">
            <span class="nav-icon material-symbols-rounded">dashboard</span>
            <span class="nav-label">Pampanga Dashboard</span>
          </a>
          <span class="nav-tooltip">Pampanga Dashboard</span>
        </li>

        <li class="nav-item">
          <a href="?page=pampanga-products-tables" class="nav-link ">
            <i class="fa-solid fa-location-dot dotted"></i>
            <span class="nav-label">Pampanga Products</span>
          </a>
          <span class="nav-tooltip">Pampanga Products</span>
        </li>
        ...
        <li class="nav-item">
          <a href="?page=releasing-item-section" class="nav-link ">
        ...
        <li class="nav-item">
          <a href="?page=Pampanga-Customer-Records" class="nav-link ">
      ...
      <ul class="nav-list secondary-nav">
        ...
        <li class="nav-item">
          <a href="?page=ppg-history" class="nav-link ">
            <i class="fa-regular fa-clock"></i> <span class="nav-label">Pampanga History</span>
          </a>
          <span class="nav-tooltip">Pampanga History</span>
        </li>
  ...
  <div class="main">
    ...
    switch ($page) {
      ...
      case 'pampanga_dashboard':
        include 'pmpng_db.php';
        break;

      case 'pampanga-products-tables':
        include 'pmgpng_prod_tbl.php';
        break;

      case 'add-products':
        include 'pmpng_add_prod.php';
        break;

      case 'releasing-item-section':
        include 'pmpng_release.php';
        break;

      case 'Pampanga-Customer-Records':
        include 'pmpng_records.php';
        break;

      case 'ppg-history':
        include 'ppg-history.php';
        break;
  ...
  <script>
    ...
    window.addEventListener("beforeunload", function () {
      ...
      if (!isNavigatingAway && !isReload && !isBackForward) {
        navigator.sendBeacon("pampanga/logout-on-close.php");
      }
    });
  </script>

  <script src="pampanga/js/jquery.js"></script>
  <script src="pampanga/js/main.js"></script>
