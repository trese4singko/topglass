...
if (isset($_GET['search'])) {
    ...
    $query = "SELECT name FROM pmpng_add_prod_data WHERE name LIKE '$searchTerm' LIMIT 5";
...
if (isset($_POST['check_availability'])) {
    ...
    $checkProduct = "SELECT quantity_on_hand FROM pmpng_add_prod_data WHERE name='$productName' AND size='$productSize'";
...
$customerQuery = "INSERT INTO pmpng_customer (name, contact, address) VALUES ('$customerName', '$customerNum', '$customerAdd')";
...
$orderQuery = "INSERT INTO pmpng_orders (customer_id) VALUES ($customerID)";
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    if ($currentQuantity >= $quantity) {
        ...
        $updateStock = "UPDATE pmpng_add_prod_data SET quantity_on_hand= $newQuantity WHERE name='$productName' AND size='$productSize'";
        ...
        if (mysqli_query($con, $orderItemQuery)) {
            ...
            logppgActivity($con, $username, 'Release Item', null, $description); // Log the activity
