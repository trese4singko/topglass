    ...
    try {
        ...
        foreach ($data as $row) {
            ...
            if (mysqli_num_rows($checkQuery) > 0) {
                ...
                $updateQuery = mysqli_query($con, "UPDATE pmpng_add_prod_data SET quantity_on_hand='$newQuantity', status='$status' WHERE product_id='{$existingProduct['product_id']}'");
                ...
                // Insert new product
                $query = "INSERT INTO pmpng_add_prod_data (name, size, price, quantity_on_hand, category, status)
