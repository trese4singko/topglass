...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $stmt = $con->prepare("SELECT name FROM pmpng_add_prod_data WHERE name LIKE ? LIMIT 5");
    $searchTerm = "%$search%";
...
if (isset($_POST['check_availability'])) {
    ...
    $checkProduct = "SELECT quantity_on_hand FROM pmpng_add_prod_data WHERE name=? AND size=?";
...
$customerQuery = "INSERT INTO pmpng_customer (name, contact, address) VALUES (?, ?, ?)";
...
$orderQuery = "INSERT INTO pmpng_orders (customer_id) VALUES (?)";
...
// Get the correct order_id using last_insert_id
$orderID = $con->insert_id; // This should be correct now
...
if ($result->num_rows > 0) {
    ...
    if ($currentQuantity >= $quantity) {
        ...
        $updateStock = "UPDATE pmpng_add_prod_data SET quantity_on_hand=? WHERE name=? AND size=?";
        $stmt = $con->prepare($updateStock);
        $stmt->bind_param("iss", $newQuantity, $productName, $productSize);
        $stmt->execute();

        $orderItemQuery = "INSERT INTO pmpng_order_items (order_id, product_name, size, quantity, price, total_price) 
                           VALUES (?, ?, ?, ?, ?, ?)";
        ...
        if ($stmt->execute()) {
            ...
            $description = "Released $quantity x $productName ($productSize) to customer '$customerName' [Order ID: $orderID]";
            logActivity($con, $username, 'Release Item', $orderID, $description);
            logppgActivity($con, $username, 'Release Item', $orderID, $description);
