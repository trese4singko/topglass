...
if (isset($_POST['order_id'])) { 
    ...
    if ($orderResult && mysqli_num_rows($orderResult) > 0) {
        ...
        if ($deletequery) {
            ...
            $description = "Deleted order items for order ID '$id' (Customer: '$customerName', Product: '$productName in Taguig Release Records')";

            logActivity($con, $username, 'Delete Order Item', $id, $description);
            logtgActivity($con, $username, 'Delete Order Item', $id, $description); // Log to branch-specific activity log
