...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    function handleLogin($con, $row, $table)
    {
        ...
        logtgActivity($con, $row['username'], 'Login', null, "Admin user {$row['username']} has logged in Taguig Branch");
        logActivity($con, $row['username'], 'Login', null, "Admin user {$row['username']} has logged in Taguig Branch");
    ...
    function checkLoginTable($con, $table)
    {
        ...
        if ($res && mysqli_num_rows($res) > 0) {
            ...
            if ($row['usertype'] !== 'Taguig Admin') {
                echo json_encode(["status" => "unauthorized", "message" => "Access restricted. For Taguig admins/Employees only."]);
                exit();
            }
            ...
            if (password_verify($password, $row['password'])) {
                ...
                logActivity($con, $username, false, "Wrong password for $username");
