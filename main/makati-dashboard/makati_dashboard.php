...
$action = "Super Admin viewed on Makati Dashboard";
...
// Taguig stats
$result = mysqli_query($con, "SELECT COUNT(*) AS total_products FROM mkt_add_prod_data");
$row = mysqli_fetch_assoc($result);
$total_products = $row['total_products'];

// Total inventory value for makati
$salesValueResult = mysqli_query($con, "SELECT SUM(total_price) AS total_sales FROM makati_order_items");
$salesValueRow = mysqli_fetch_assoc($salesValueResult);
$inventory_value = $salesValueRow['total_sales'] ?? 0;


$lowStockResult = mysqli_query($con, "SELECT COUNT(*) AS low_stock FROM mkt_add_prod_data WHERE quantity_on_hand <= 0");
$lowStockRow = mysqli_fetch_assoc($lowStockResult);
$low_stock = $lowStockRow['low_stock'];

$noStockResult = mysqli_query($con, "SELECT COUNT(*) AS no_stock FROM mkt_add_prod_data WHERE quantity_on_hand BETWEEN 1 AND 20");
...
<head>
    ...
    <!-- Custom CSS -->
    <link rel="stylesheet" href="css\makati_db.css">
</head>
