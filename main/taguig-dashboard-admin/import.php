    ...
    try {
        ...
        foreach ($data as $row) {
            ...
            if (mysqli_num_rows($checkQuery) > 0) {
                ...
                $updateQuery = mysqli_query($con, "UPDATE producttabletaguig SET quantity_on_hand='$newQuantity', status='$status' WHERE product_id='{$existingProduct['product_id']}'");
                
                if ($updateQuery) {
                    $insertCount++;
                }
            } else {
                $query = "INSERT INTO producttabletaguig (name, size, price, quantity_on_hand, category, status)
