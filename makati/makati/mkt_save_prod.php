    ...
    foreach ($products as $product) {
        ...
        if (mysqli_num_rows($checkQuery) > 0) {
            ...
            $insertQuery = mysqli_query(
                $con,
                "INSERT INTO mkt_add_prod_data (name, size, price, quantity_on_hand, category, status) VALUES 
            ...
            logmktActivity(
                ...
                'ADD',
                $newId,
                "Added new product: '$name' ($size), qty: $quantity_on_hand on Makati Add Products"
            );

            // Log the activity for adding the product
            logActivity($con, $username, 'Add Item', $newId, "Added new product: $name ($size), qty: $quantity_on_hand on Taguig Add Products");
        }
    }
