<?php
// If user is already logged in, redirect to main system
if (isset($_SESSION['userid']) && isset($_SESSION['session_token'])) {
    header("Location: taguig-dashboard-admin/index.php");
    exit();
}
...
include_once 'connection.php'; // Adjust if your path is different
...
if (isset($_SESSION['userid']) && isset($_SESSION['session_token'])) {
    ...
    if (
        (!isset($row1['session_token']) || $row1['session_token'] !== $session_token)
    ) {
        ...
        header("Location: taguig-dashboard-admin/index.php?page=taguig_dashboard");  // Adjust this path to your main page
        exit();
    }
}
...
<head>
    ...
    <link rel="stylesheet" href="taguig-dashboard-admin/taguig_css/login.css">
...
<body>
                ...
                <div class="texts">
                    ...
                    <form class="user" method="POST" autocomplete="off">
                        <div class="form-group">
                            <input type="text" class="username both" id="username" name="username"
                                placeholder="Enter Username" required>
                        </div>
                        <div class="form-group">
                            <input type="password" class="password both" name="password" id="password"
                        ...
                        <div id="lockout-timer"
                            style="color: red; text-align:center; display: none; margin-bottom: 10px;"></div>
    ...
    <script>
            ...
            fetch("login_process.php", {
                ...
                .then(data => {
                    if (data.status === "already_logged_in") {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Access Denied. This link is allowed for Taguig admin only.',
                            text: data.message,
                        });
                    } else if (data.status === "locked") {
                        const duration = data.lockout_remaining || 600;
        ...
        function showLockoutModal(duration) {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Page Locked Due to Multiple Failed Attempts <h6>Wait for 10Mins before trying Again</h6> ',
